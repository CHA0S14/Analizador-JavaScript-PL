/* Programa de ejemplo  */
/******* Jose Luis Fuertes, 5, enero, 2018 *********/
...
function int FactorialRecursivo (int n)	/* n parametro formal de la funcion entera */
{
	if (n == 0)	return 1;
	return n * FactorialRecursivo (n - 1);	/* llamada recursiva */
}

function int FactorialDo (int n)   ----> Ultimo token que lee y luego da error por  imagino que el de la linea 18
{
...
{
	var int factorial = 1, i;	// variables locales factorial inicializada a 1 e i inicializada a 0 por omision
...
function imprime (chars s, chars msg, int f)	/* funcion que recibe 3 argumentos */
{
	write (s); write (msg); write (f);
	write ("\n");	// imprime un salto de linea */
	return;	/* finaliza la ejecucion de la funcion (en este caso, se podria omitir) */
}
...
}	// fin cadena funcion que devuelve una cadena
...
} // fin de bisiesto funcion logica
...
} // fin de potencia funcion que no devuelve nada

function demo ()	/* definicion de la funcion demo, sin argumentos y que no devuelve nada */
{
